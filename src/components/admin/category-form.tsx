 ...
 * against the API's Zod layer — so the two admin forms stay predictable against
 * each other.
 *
 * One deliberate difference: here the slug stops following the name as soon as
 * the category has been saved once. Product addresses now re-derive on a rename
 * because a product's URL is what customers share; a category slug is a
 * navigation path with children hanging off it, so it is left where it is.
 */
...
export function CategoryFormDialog({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      // Only sub-categories reach the homepage rail, so a parent with Homepage
      // ticked changes nothing — say what actually happened, not what was asked.
      done(
        isEdit ? "Saved" : "Created",
        form.featured && form.parentId !== ""
          ? `“${form.name}” is saved and showing on the homepage.`
          : `“${form.name}” is saved.`,
      );
